...
async function main() {
  ...
  try {
    for (const team of WORLD_CUP_TEAMS) {
      console.log(`\n🌍 Processing Away Kit: ${team.team} (${team.teamId})`);
      ...
      const kitLinks = await page.$$eval("a", (anchors) => 
        anchors
          ...
          .filter(item => item.href.includes("-kit-") || item.href.includes("-away-"))
      );

      // Find best Away kit link
      const awayKitLink = kitLinks.find(link => 
        (link.text.toLowerCase().includes("away") || link.href.toLowerCase().includes("away")) &&
      ...
      ) ?? kitLinks.find(link => link.text.toLowerCase().includes("away") || link.href.toLowerCase().includes("away"));

      if (!awayKitLink) {
        console.warn(`  ⚠️ No recent away kit link found for ${team.team}. Skipping.`);
        continue;
      }

      console.log(`  🔗 Found Away Kit Link: "${awayKitLink.text}" -> ${awayKitLink.href}`);
      
      await page.goto(awayKitLink.href, { waitUntil: "domcontentloaded", timeout: 45000 });
      await waitForBypass(page);
      await page.waitForTimeout(1000);
      ...
      const storagePath = `international/${team.teamId}/away.jpg`;
  ...
  console.log("\n=== CDP Away Kit Scraping Completed! ===");
  fs.writeFileSync("scratch/flat_lay_away_results.json", JSON.stringify(results, null, 2));
  console.log("Saved results to scratch/flat_lay_away_results.json");
}
