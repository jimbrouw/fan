...
*   **`/api/captures` (upload)**: now requires auth and ties the capture session to `user_id` (was unauthenticated, writing `user_id: null`).
...
*   **Residual (low, follow-up)**: the capture storage path is `${sessionId}/${type}.jpg`; a holder of another user's session UUID could overwrite captures. The auth gate blocks anonymous abuse; per-session ownership verification is a follow-up.
